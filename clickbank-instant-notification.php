<?php
function execute_clickbank_instant_notification()
{
	global $wpdb;
	...
	$page_name = APM_PHYSICAL_PATH."/logs/clickbank_debug_".date("d-m-Y").".txt";	
	$fp = fopen($page_name, "a");
	$dataArr = print_r($_REQUEST, TRUE);
	fwrite($fp, $dataArr);
	
	if(isset($_REQUEST['mode']) && $_REQUEST['mode'] == 'clickbank_notification')
	{
		//--> Debug Tracking Code
		...
		$sql = $wpdb->prepare("INSERT INTO `".$wpdb->prefix."cso_my_income`
						 ...
						 VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'Yes', 'ClickBank')",
